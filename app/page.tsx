...
export default function InventoryDashboard() {
  ...
  const handleAddItem = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.from('items').insert([{ name: newName, category: newCategory, current_stock: parseInt(newStock, 10) || 0, safety_stock: parseInt(newSafetyStock, 10) || 50, is_active: true }]);
  ...
  return (
    /* 💡 모바일에서는 flex-col(상하 배치), PC(lg 이상)에서는 flex-row(좌우 배치)로 자동 전환 */
    <div className="flex flex-col lg:flex-row h-screen bg-[#0B0F19] text-gray-200 font-sans overflow-hidden">
      
      {/* 🟢 좌측(PC) / 상단(모바일): 메인 대시보드 영역 */}
      <div className="flex-1 p-4 lg:p-8 overflow-y-auto">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 gap-3">
          <h1 className="text-xl lg:text-2xl font-bold text-white tracking-tight">전체 재고 관리 대시보드</h1>
          <button onClick={() => setIsAddModalOpen(true)} className="px-4 py-2 bg-blue-600 hover:bg-blue-500 font-bold rounded-lg text-white shadow-lg text-sm whitespace-nowrap">+ 새 품목 등록</button>
        </div>
        ...
        <div className="mb-4 lg:mb-6">
          <input 
            type="text" 
            placeholder={viewMode === 'active' ? "현재 취급 품목 실시간 검색..." : "식약처 전체 실시간 검색..."}
                ...
                <tr key={item.id} className={`border-b border-gray-800/60 hover:bg-gray-800/30 transition-all duration-150 group ${!item.is_active && 'opacity-50'}`}>
                  ...
                  <td className="py-4 font-medium text-white">
                    <div className="flex items-center">
                      ...
                      {/* 모바일(터치)을 고려해 아이콘 상시 노출(lg 이상에서만 hover로 숨김) */}
                      <button 
                        onClick={() => { setSelectedItem(item); setEditName(item.name); setIsEditModalOpen(true); }}
                        className="ml-2 opacity-100 lg:opacity-0 lg:group-hover:opacity-100 text-gray-400 hover:text-blue-400 transition-all duration-150 p-1.5 rounded hover:bg-gray-800 bg-gray-800/50 lg:bg-transparent"
                        title="품목명 수정"
                      >
                  ...
                  <td className="py-4 text-center">
                    <div className="inline-flex items-center space-x-3 bg-[#0B0F19]/40 px-3 py-1.5 rounded-lg border border-gray-800">
                      ...
                      <div className="flex items-center space-x-1.5">
                        <input type="number" min="0" placeholder="대량" disabled={!item.is_active} className="w-16 p-1.5 text-center rounded bg-[#0B0F19] border border-gray-700 text-white focus:outline-none focus:border-blue-500 disabled:opacity-30 text-xs" value={inputValues[item.id] || ''} onChange={(e) => handleInputChange(item.id, e.target.value)} />
      ...
      {/* 🔴 우측(PC) / 하단(모바일): 긴급 보충 피드 */}
      {/* 💡 모바일에서는 화면 높이의 40%를 차지하도록 h-[40vh] 부여 */}
      <div className="w-full lg:w-96 h-[40vh] lg:h-auto bg-[#111827] border-t lg:border-t-0 lg:border-l border-gray-800/80 p-4 lg:p-6 flex flex-col shadow-2xl shrink-0 z-10">
        <div className="flex items-center mb-3 lg:mb-4 border-b border-gray-800 pb-3">
        ...
        <div className="mb-3 lg:mb-4">
          <input 
            type="text" placeholder="부족 품목 중 검색..." 
            className="w-full p-2.5 rounded-lg bg-[#0B0F19] border border-gray-700 text-white placeholder-gray-500 focus:outline-none focus:border-red-500 transition-colors text-xs shadow-inner"
        ...
        <div className="flex-1 overflow-y-auto space-y-2.5 pr-1 scrollbar-thin">
          {filteredShortageItems.length === 0 ? (
            <div className="text-center text-gray-500 mt-6 text-xs lg:text-sm">부족한 품목이 없거나 검색 결과가 없습니다.</div>
          ) : (
            filteredShortageItems.map(item => (
              <div key={item.id} className="bg-red-950/20 border border-red-900/50 p-3 lg:p-4 rounded-xl flex flex-col shadow-md">
      ...
      {isEditModalOpen && selectedItem && (
        <div className="fixed inset-0 bg-black/85 flex items-center justify-center z-50 p-4 backdrop-blur-md">
          <form onSubmit={handleUpdateName} className="bg-[#111827] border border-gray-800 rounded-2xl p-5 lg:p-6 max-w-sm w-full shadow-2xl space-y-4">
            <h3 className="text-base font-bold text-white pb-3 border-b border-gray-800">✏️ 품목 명칭 변경</h3>
            <div>
              <label className="block text-xs text-gray-400 mb-1.5">선택한 품목 고유코드: #{selectedItem.id}</label>
              <input type="text" required className="w-full p-2.5 rounded-lg bg-[#0B0F19] border border-gray-700 text-white text-sm focus:outline-none focus:border-blue-500" value={editName} onChange={(e) => setEditName(e.target.value)} />
            </div>
            <div className="flex space-x-3 justify-end pt-2 border-t border-gray-800">
              <button type="button" onClick={() => setIsEditModalOpen(false)} className="px-4 py-2 rounded-lg bg-gray-800 text-xs font-medium text-white hover:bg-gray-700">취소</button>
      ...
      {isAddModalOpen && (
        <div className="fixed inset-0 bg-black/85 flex items-center justify-center z-50 p-4 backdrop-blur-md">
          <form onSubmit={handleAddItem} className="bg-[#111827] border border-gray-800 rounded-2xl p-5 lg:p-6 max-w-md w-full space-y-4">
            ...
            <div className="grid grid-cols-3 gap-2">
              <div>
                ...
                <select className="w-full p-2 rounded-lg bg-[#0B0F19] border border-gray-700 text-white text-sm" value={newCategory} onChange={(e) => setNewCategory(e.target.value)}><option value="의약품">의약품</option><option value="의료기재">의료기재</option></select>
              </div>
              <div>
                <label className="block text-xs text-gray-400 mb-1.5">초기고</label>
                <input type="number" className="w-full p-2 rounded-lg bg-[#0B0F19] border border-gray-700 text-white text-sm text-center" value={newStock} onChange={(e) => setNewStock(e.target.value)} />
              </div>
              <div>
                <label className="block text-xs text-gray-400 mb-1.5">안전재고</label>
                <input type="number" className="w-full p-2 rounded-lg bg-[#0B0F19] border border-gray-700 text-white text-sm text-center" value={newSafetyStock} onChange={(e) => setNewSafetyStock(e.target.value)} />
              </div>
            </div>
            <div className="flex space-x-3 justify-end pt-2 border-t border-gray-800">
              <button type="button" onClick={() => setIsAddModalOpen(false)} className="px-4 py-2 rounded-lg bg-gray-800 text-sm text-white">취소</button>
      ...
      {isModalOpen && selectedItem && (
         <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4 backdrop-blur-sm">
          <div className="bg-[#111827] border border-gray-800 rounded-2xl p-5 lg:p-6 max-w-sm w-full shadow-2xl">
            ...
            <div className="flex space-x-3 justify-end mt-5">
              <button onClick={() => setIsModalOpen(false)} className="px-4 py-2 rounded-lg bg-gray-800 text-sm text-white">취소</button>
